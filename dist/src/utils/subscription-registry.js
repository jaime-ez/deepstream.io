...
class SubscriptionRegistry {
     ...
     * than functions
     */
    constructor(config, services, topic, clusterTopic) {
        this.pending = [];
        this.delay = -1;
        ...
        switch (topic) {
            case constants_1.TOPIC.RECORD:
                this.actions = constants_1.RECORD_ACTIONS;
                break;
            case constants_1.TOPIC.EVENT:
                this.actions = constants_1.EVENT_ACTIONS;
                break;
            case constants_1.TOPIC.RPC:
                this.actions = constants_1.RPC_ACTIONS;
                break;
            case constants_1.TOPIC.PRESENCE:
                this.actions = constants_1.PRESENCE_ACTIONS;
                break;
        }
        this.constants = {
     ...
     * via the cluster.
     */
    setupRemoteComponents(clusterTopic) {
     ...
     * broadcast.
     */
    sendToSubscribers(name, message, noDelay, socket, isRemote = false) {
        if (socket && !isRemote) {
    ...
    subscribe(message, socket, silent) {
        ...
        if (subscription.sockets.size === 0) {
            ...
            this.services.logger.warn(constants_1.EVENT_ACTIONS[this.constants.MULTIPLE_SUBSCRIPTIONS], msg);
            socket.sendMessage({
                topic: this.topic,
        ...
        if (!silent) {
            const logMsg = `for ${constants_1.TOPIC[this.topic]}:${name} by ${socket.user}`;
...
exports.default = SubscriptionRegistry;
