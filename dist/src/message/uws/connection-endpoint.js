...
const messageBuilder = require("../../../binary-protocol/src/message-builder");
...
class UWSConnectionEndpoint extends events_1.EventEmitter {
    ...
    _uwsInit() {
        ...
        uws.native.server.group.startAutoPing(this.serverGroup, 2 * this._getOption('heartbeatInterval'), this.pingMessage);
    ...
    _handleParseErrors(socketWrapper, parseResults) {
        ...
        for (const parseResult of parseResults) {
            ...
            if (message.topic === constants_1.TOPIC.CONNECTION &&
                message.action === constants_1.CONNECTION_ACTIONS.PONG) {
                continue;
            }
    ...
    */
    ...
    _processConnectionMessage(socketWrapper, parsedMessages) {
        ...
        if (msg.action === constants_1.CONNECTION_ACTIONS.CHALLENGE) {
