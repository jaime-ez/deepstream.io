...
describe('record request', () => {
    ...
    describe('records are requested from cache and storage sequentially', () => {
        ...
        it('returns null for non existent records', () => {
            services.cache.nextGetWillBeSynchronous = true;
            recordRequest('doesNotExist', config, services, client.socketWrapper, completeCallback, errorCallback, null);
        ...
        it('handles cache errors', () => {
            ...
            recordRequest('cacheError', config, services, client.socketWrapper, completeCallback, errorCallback, null);
            expect(errorCallback).toHaveBeenCalledWith(C.RECORD_ACTIONS.RECORD_LOAD_ERROR, 'error while loading cacheError from cache:storageError', 'cacheError', client.socketWrapper);
            expect(completeCallback).not.toHaveBeenCalled();
            expect(services.logger._log).toHaveBeenCalledWith(3, C.RECORD_ACTIONS.RECORD_LOAD_ERROR, 'error while loading cacheError from cache:storageError');
        ...
        it('handles storage errors', () => {
            ...
            recordRequest('storageError', config, services, client.socketWrapper, completeCallback, errorCallback, null);
            expect(errorCallback).toHaveBeenCalledWith(C.RECORD_ACTIONS.RECORD_LOAD_ERROR, 'error while loading storageError from storage:storageError', 'storageError', client.socketWrapper);
            expect(completeCallback).not.toHaveBeenCalled();
        ...
        describe('handles cache timeouts', () => {
            ...
            it('sends a CACHE_RETRIEVAL_TIMEOUT message when cache times out', done => {
                recordRequest('willTimeoutCache', config, services, client.socketWrapper, completeCallback, errorCallback, null);
                setTimeout(() => {
                    expect(errorCallback).toHaveBeenCalledWith(C.RECORD_ACTIONS.CACHE_RETRIEVAL_TIMEOUT, 'willTimeoutCache', 'willTimeoutCache', client.socketWrapper);
                    expect(completeCallback).not.toHaveBeenCalled();
        ...
        describe('handles storage timeouts', () => {
            ...
            it('sends a STORAGE_RETRIEVAL_TIMEOUT message when storage times out', done => {
                recordRequest('willTimeoutStorage', config, services, client.socketWrapper, completeCallback, errorCallback, null);
                setTimeout(() => {
                    expect(errorCallback).toHaveBeenCalledWith(C.RECORD_ACTIONS.STORAGE_RETRIEVAL_TIMEOUT, 'willTimeoutStorage', 'willTimeoutStorage', client.socketWrapper);
                    expect(completeCallback).not.toHaveBeenCalled();
    ...
    describe('excluded records are not put into storage', () => {
        ...
        it('returns null when requesting a record that doesn\'t exists in a synchronous cache, and is excluded from storage', done => {
            recordRequest('dont-save/1', config, services, client.socketWrapper, completeCallback, errorCallback, null);
            expect(completeCallback).toHaveBeenCalledWith('dont-save/1', -1, null, client.socketWrapper);
            expect(errorCallback).not.toHaveBeenCalled();
...
//# sourceMappingURL=record-requestSpec.js.map