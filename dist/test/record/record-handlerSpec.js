...
describe('record handler handles messages', () => {
    ...
    beforeEach(() => {
        ...
        recordHandler = new RecordHandler(config, services, testMocks.subscriptionRegistry, testMocks.listenerRegistry);
    });
    afterEach(() => {
    ...
    it('creates a non existing record', () => {
        ...
        chai_1.expect(services.cache.lastSetKey).to.equal('some-record');
        chai_1.expect(services.cache.lastSetVersion).to.equal(0);
        chai_1.expect(services.cache.lastSetValue).to.deep.equal({});
        chai_1.expect(services.storage.lastSetKey).to.equal('some-record');
        chai_1.expect(services.storage.lastSetVersion).to.equal(0);
        chai_1.expect(services.storage.lastSetValue).to.deep.equal({});
    });
    ...
    it('does not store new record when excluded', () => {
        ...
        recordHandler.handle(client.socketWrapper, M.subscribeCreateAndReadMessage);
        chai_1.expect(services.storage.lastSetKey).to.equal(null);
        chai_1.expect(services.storage.lastSetVersion).to.equal(null);
        chai_1.expect(services.storage.lastSetValue).to.equal(null);
    });
    ...
    xit('patches a record', () => {
        ...
        services.cache.get('some-record', (error, record) => {
            chai_1.expect(record).to.deep.equal({ _v: 6, _d: { name: 'Kowalski', lastname: 'Egon' } });
        });
    });
    it('updates a record', () => {
        services.cache.set('some-record', M.recordVersion, M.recordData, () => { });
        ...
        services.cache.get('some-record', (error, version, result) => {
            chai_1.expect(version).to.equal(6);
            chai_1.expect(result).to.deep.equal({ name: 'Kowalski' });
        });
    });
    it('rejects updates for existing versions', () => {
        ...
        chai_1.expect(services.logger.lastLogMessage).to.equal('someUser tried to update record some-record to version 5 but it already was 5');
    });
    ...
    it('handles deletion messages', () => {
        ...
        services.cache.get(M.recordDelete.name, (error, version, data) => {
            chai_1.expect(version).to.deep.equal(-1);
            chai_1.expect(data).to.equal(null);
        });
    });
    it('updates a record with a -1 version number', () => {
        ...
        services.cache.get(M.recordUpdate.name, (error, version, data) => {
            chai_1.expect(data).to.deep.equal(M.recordUpdate.parsedData);
            chai_1.expect(version).to.equal(6);
        });
    });
    it('updates multiple updates with an -1 version number', () => {
        ...
        services.cache.get(M.recordUpdate.name, (error, version, result) => {
            chai_1.expect(result).to.deep.equal(M.recordUpdate.parsedData);
        });
    });
    xit('creates records when using CREATEANDUPDATE', () => {
        testMocks.subscriptionRegistryMock
            .expects('sendToSubscribers')
            .once()
            .withExactArgs(M.createAndUpdate.name, Object.assign({}, M.createAndUpdate, { action: C.RECORD_ACTIONS.UPDATE, version: 1 }), false, client.socketWrapper);
        recordHandler.handle(client.socketWrapper, M.createAndUpdate);
        services.cache.get(M.createAndUpdate.name, (error, version, data) => {
            chai_1.expect(version).to.deep.equal(1);
            chai_1.expect(data).to.deep.equal(M.recordData);
        });
    });
